...
| 항목 | 값 | 비고 |
|---|---|---|
| DRV2605L VDD | 2.0 ~ 5.2V | **반드시 5V 사용 (3.3V에서는 다이나믹 레인지 부족)** |
...
> ⚠️ **DRV2605L VDD는 반드시 5V 사용.**  
> 3.3V에서는 캘리브레이션 실패, 중간 진폭(50~100) 구간 비선형 현상, 다이나믹 레인지 심각히 부족 확인됨.  
> ESP32의 **VIN 핀(5V)** 또는 USB 5V에서 직접 공급할 것.  
> SDA/SCL은 3.3V 그대로 OK (DRV2605L I²C는 1.8V~VDD 톨러런트).
...
※ ESP32 3.3V 핀에서 DRV2605L VDD 공급 금지
```
